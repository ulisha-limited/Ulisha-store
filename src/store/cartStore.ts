...
export const useCartStore = create<CartState>((set, get) => ({
  ...
  initializeSession: async () => {
    try {
      ...
      } = await retryWithBackoff(async () => supabase.auth.getSession())
  ...
  addToCart: async (product: Product, quantity = 1) => {
    try {
      ...
      if (existingItem) {
        ...
        // Insert new item
        await retryWithBackoff(async () =>
          supabase.from('cart_items_new').insert([
            {
  ...
  clearCart: async () => {
    try {
      ...
      await retryWithBackoff(
        async () =>
          await supabase.from('cart_items_new').delete().eq('session_id', currentSession.id),
      )
  ...
  processPayment: async (total: number, deliveryDetails?: DeliveryDetails) => {
    try {
      ...
      const { data: order, error: orderError } = await retryWithBackoff(
        async () => await supabase.from('orders').insert([orderData]).select().single(),
      )
      ...
      await retryWithBackoff(async () => await supabase.from('order_items').insert(orderItems))
