    ...
    <div className="mt-2">
      ...
      <div className="grid grid-cols-2 gap-2">
        <div className="flex items-center text-xs text-gray-500">
          <Check
            className={`h-3 w-3 mr-1 ${password.length >= 8 ? 'text-green-500' : 'text-gray-300'}`}
          />
          <span>8+ characters</span>
        </div>
        <div className="flex items-center text-xs text-gray-500">
          <Check
            className={`h-3 w-3 mr-1 ${/[A-Z]/.test(password) ? 'text-green-500' : 'text-gray-300'}`}
          />
          <span>Uppercase</span>
        </div>
        <div className="flex items-center text-xs text-gray-500">
        ...
        <div className="flex items-center text-xs text-gray-500">
          <Check
            className={`h-3 w-3 mr-1 ${/[0-9]/.test(password) ? 'text-green-500' : 'text-gray-300'}`}
          />
          <span>Number</span>
        </div>
      </div>
