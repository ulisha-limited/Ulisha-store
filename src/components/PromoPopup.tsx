...
export function PromoPopup({ isVisible, onClose }: PromoPopupProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
      ...
      <div className="md:hidden bg-white rounded-2xl max-w-sm w-full mx-4 overflow-hidden shadow-2xl transform transition-all animate-fade-in">
        {/* Header */}
      ...
      <div className="hidden md:block">
        <div className="bg-white rounded-2xl max-w-4xl w-full mx-4 overflow-hidden shadow-2xl transform transition-all animate-fade-in">
          {/* Header */}
          <div className={`relative ${isSecondShow 
            ? 'bg-gradient-to-r from-purple-600 to-pink-600' 
            : 'bg-gradient-to-r from-primary-orange to-red-500'
          ...
          {/* Product Showcase - Landscape Layout */}
          <div className="p-6">
            {loading ? (
              <div className="flex items-center justify-center py-8">
                <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-primary-orange"></div>
              ...
              <div className="grid grid-cols-2 gap-6 items-center">
                ...
                <div className="relative">
                  ...
                  />
                  {featuredProduct.discount_active && featuredProduct.discount_percentage && (
                    <div className="absolute top-3 left-3 bg-red-500 text-white px-3 py-1 rounded-full flex items-center space-x-1">
                      <Percent className="w-4 h-4" />
                      <span className="text-sm font-bold">{featuredProduct.discount_percentage}% OFF</span>
                    </div>
                  )}
                  {isSecondShow && (
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <div className="flex items-center mb-4">
                      <div className="flex items-center text-orange-400">
                        {[...Array(5)].map((_, i) => (
                          <Star key={i} className={`w-5 h-5 ${(featuredProduct.rating || 5) > i ? 'fill-current' : 'text-gray-300'}`} />
                        ))}
                      </div>
                      <span className="text-sm text-gray-500 ml-2">({featuredProduct.rating || 5})</span>
                    </div>
                  ...
                  <div className="space-y-3">
                    <button
                      ...
                      className={`w-full ${isSecondShow 
                        ? 'bg-purple-600 hover:bg-purple-700' 
                        : 'bg-primary-orange hover:bg-primary-orange/90'
                      } text-white py-3 px-6 rounded-lg font-medium transition-colors flex items-center justify-center space-x-2`}
                    >
                      <ShoppingBag className="w-5 h-5" />
                      <span>{isSecondShow ? 'Grab This Deal' : 'Shop Now'}</span>
                    ...
                    <div className="text-center">
                      ...
                      <button
                        ...
                        className={`w-full ${isSecondShow 
                          ? 'bg-gradient-to-r from-pink-500 to-purple-600 hover:from-pink-600 hover:to-purple-700' 
                          : 'bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700'
                        } text-white py-3 px-6 rounded-lg font-medium transition-all transform hover:scale-105`}
                      >
                        {isSecondShow ? 'Register & Save More!' : 'Register for Exclusive Discounts'}
                      </button>
                    </div>
                  ...
                  {/* Benefits */}
                  <div className={`p-4 ${isSecondShow ? 'bg-purple-50' : 'bg-gray-50'} rounded-lg`}>
              ...
              <div className="text-center py-8">
                ...
                <button
                  onClick={handleRegister}
                  className={`${isSecondShow 
                    ? 'bg-purple-600 hover:bg-purple-700' 
                    : 'bg-primary-orange hover:bg-primary-orange/90'
                  } text-white py-3 px-8 rounded-lg font-medium transition-colors`}
                >
                  {isSecondShow ? 'Register Now - Final Chance!' : 'Register Now'}
                </button>
              </div>
