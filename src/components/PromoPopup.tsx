  ...
  const fetchRandomDiscountedProduct = async () => {
    try {
      ...
      if (discountedProducts && discountedProducts.length > 0) {
        ...
        const { data: allProducts } = await supabase.from('products').select('*').limit(10);
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
      ...
      <div className="md:hidden bg-white rounded-2xl w-[90%] max-h-[90vh] overflow-y-auto shadow-2xl animate-fade-in">
        <div className={`${isSecondShow ? 'bg-gradient-to-r from-purple-600 to-pink-600' : 'bg-gradient-to-r from-primary-orange to-red-500'} p-4 text-white relative`}>
          <button onClick={onClose} className="absolute top-3 right-3">
            <X className="w-5 h-5" />
          </button>
          <div className="text-center">
            <div className="flex items-center justify-center mb-2">
              {isSecondShow ? <Gift className="w-6 h-6 mr-1" /> : <ShoppingBag className="w-6 h-6 mr-1" />}
            ...
            <p className="text-sm opacity-90">
              {isSecondShow ? "Don't miss out on these exclusive discounts" : 'Register now and enjoy exclusive discounts'}
            </p>
          </div>
        </div>
        ...
        <div className="p-4 text-center">
          {loading ? (
            <div className="py-6"><div className="animate-spin h-6 w-6 border-b-2 border-primary-orange rounded-full mx-auto"></div></div>
          ) : featuredProduct ? (
            <>
              <div className="relative mb-4">
                <img src={featuredProduct.image} alt={featuredProduct.name} className="w-full h-36 object-cover rounded-lg" />
              ...
              <h3 className="text-sm font-semibold text-gray-900 mb-1 line-clamp-2">{featuredProduct.name}</h3>
              <div className="flex justify-center items-center text-orange-400 mb-2">
                ...
                <span className="text-xs text-gray-500 ml-1">({featuredProduct.rating || 5})</span>
              </div>
              <div className="text-primary-orange font-bold text-lg mb-3">
              ...
              {featuredProduct.original_price && (
                <div className="flex justify-center items-center text-xs text-gray-500 line-through mb-2">
                  ...
                  <span className="text-green-600 font-semibold ml-2">Save {featuredProduct.discount_percentage}%</span>
                </div>
              )}
              <button onClick={handleShopNow} className={`w-full ${isSecondShow ? 'bg-purple-600' : 'bg-primary-orange'} hover:opacity-90 text-white py-2 px-3 rounded-lg flex justify-center items-center space-x-2`}>
                <ShoppingBag className="w-4 h-4" /><span>{isSecondShow ? 'Grab This Deal' : 'Shop Now'}</span><ArrowRight className="w-4 h-4" />
              </button>
              <p className="text-xs text-gray-600 mt-2 mb-2">
                {isSecondShow ? 'Join thousands of happy customers!' : 'Want to see more amazing deals?'}
              </p>
              <button onClick={handleRegister} className={`w-full ${isSecondShow ? 'bg-gradient-to-r from-pink-500 to-purple-600' : 'bg-gradient-to-r from-blue-500 to-blue-600'} text-white py-2 px-3 rounded-lg font-medium transition-all hover:scale-105`}>
      ...
      <div className="hidden md:flex w-[85%] max-w-3xl bg-white rounded-2xl overflow-hidden shadow-2xl animate-fade-in">
        <div className="flex w-full">
          <div className="w-1/2 relative p-4">
            {featuredProduct && (
              <>
                <img src={featuredProduct.image} alt={featuredProduct.name} className="w-full h-52 object-cover rounded-lg" />
                {featuredProduct.discount_active && (
          ...
          <div className="w-1/2 p-4 relative">
            ...
            <div className="text-left">
              ...
              {featuredProduct && (
                <>
                  <h3 className="text-base font-semibold mb-1 line-clamp-2">{featuredProduct.name}</h3>
                  ...
                  <div className="text-primary-orange font-bold text-xl mb-2">
                    {formatPrice(featuredProduct.price)}
                  </div>
                  {featuredProduct.original_price && (
                    <div className="text-sm text-gray-500 line-through mb-2">
                      {formatPrice(featuredProduct.original_price)}
                      <span className="text-green-600 font-medium ml-2">Save {featuredProduct.discount_percentage}%</span>
                    </div>
                  )}
                  <button onClick={handleShopNow} className={`w-full ${isSecondShow ? 'bg-purple-600' : 'bg-primary-orange'} hover:opacity-90 text-white py-2 px-4 rounded-lg flex justify-center items-center space-x-2`}>
                    <ShoppingBag className="w-4 h-4" /><span>{isSecondShow ? 'Grab This Deal' : 'Shop Now'}</span><ArrowRight className="w-4 h-4" />
                  </button>
                  <p className="text-xs text-gray-600 mt-2 mb-2">
                    {isSecondShow ? 'Join thousands of happy customers!' : 'Want to see more amazing deals?'}
                  </p>
                  <button onClick={handleRegister} className={`w-full ${isSecondShow ? 'bg-gradient-to-r from-pink-500 to-purple-600' : 'bg-gradient-to-r from-blue-500 to-blue-600'} text-white py-2 px-4 rounded-lg font-medium transition-all hover:scale-105`}>
                    {isSecondShow ? 'Register & Save More!' : 'Register for Exclusive Discounts'}
                  </button>
                  <div className={`mt-4 p-3 ${isSecondShow ? 'bg-purple-50' : 'bg-gray-50'} rounded-lg`}>
                    <p className="text-xs text-gray-600 text-center">
