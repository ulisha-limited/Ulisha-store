...
export function MyStore() {
  ...
  const handleStoreSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      } = supabase.storage.from('store-images').getPublicUrl(logoName)
      ...
      } = supabase.storage.from('store-images').getPublicUrl(bannerName)
  ...
  const handleProductSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      } = supabase.storage.from('product-images').getPublicUrl(mainImageName)
      ...
      showNotification(
        `Product "${newProduct.name}" added successfully! Share it with your customers.`,
        'success',
      )
    } catch (error) {
            ...
            <div className="p-6">
              <div className="flex flex-col sm:flex-row items-start sm:items-center space-y-4 sm:space-y-0 sm:space-x-6">
                <img
                  src={store.logo}
                  alt={store.name}
                  className="w-24 h-24 rounded-full object-cover border-4 border-white shadow-md -mt-12 sm:-mt-16"
                />
                <div>
                      ...
                      <div className="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-40 transition-all duration-200 flex items-center justify-center opacity-0 group-hover:opacity-100">
                        <div className="flex space-x-2">
                          ...
                          <button
                            onClick={() =>
                              setShowShareOptions(
                                product.id === showShareOptions ? null : product.id,
                              )
                            }
